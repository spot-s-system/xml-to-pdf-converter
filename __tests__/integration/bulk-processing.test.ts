...
describe('integration: 月額変更_70歳以上含む.zip', async () => {
  ...
  it.skipIf(!has)(
    ...
    async () => {
      ...
      // 7210001: 70歳以上版 — 通知書名でゆるくマッチ
      // ※ 現状、日付プレフィックスは "令和07年11月_" のように出る（ゼロ埋め＋"改定"欠落）。
      //    UI ドキュメントは "令和7年11月改定_…" を謳っており、これは pdf-naming のバグ。
      //    xml-info-extractor.convertEraCode が <![CDATA[令和]]> フルテキストを R に変換できず、
      //    formatEraDateForFilename がフォールスルーするため。別途修正対象。
      //    ファイル名長制限で末尾も切り詰められるので、通知書名は前方一致で確認する。
      expect(pdfs.some((p) =>
        /厚生年金保険70歳以上被用者標準報酬月額相当額改/.test(p)
          && /様他1名_/.test(p)
      )).toBe(true);
