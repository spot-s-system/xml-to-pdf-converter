...
export function extractFromSocialInsurance(
  ...
  if (insurerBlocks) {
    ...
    insurerBlocks.forEach((block) => {
      // 名前抽出の優先順位:
      //   1. 被用者漢字氏名 (70歳以上)
      //   2. 被保険者漢字氏名 / 被保険者氏名 / 被保険者の漢字氏名 (N7027001 養育期間特例)
      //   3. 被用者カナ氏名  (70歳以上 漢字登録なし向けフォールバック)
      //   4. 被保険者カナ氏名 / 被保険者のカナ氏名 (外国籍など漢字登録なしのケース向けフォールバック)
      //
      ...
      //
      // N7027001 (養育期間標準報酬月額特例申出受理通知書) はタグ名に「の」
      // 助詞が入る (`<被保険者の漢字氏名>` / `<被保険者のカナ氏名>`) ため、
      // `の?` で吸収する。
      ...
      let name =
        ...
        pickName(/<被保険者の?(?:漢字)?氏名>(?:<!\[CDATA\[)?(.*?)(?:\]\]>)?<\/被保険者の?(?:漢字)?氏名>/);
      if (!name) {
        name =
          pickName(/<被用者カナ氏名>(?:<!\[CDATA\[)?(.*?)(?:\]\]>)?<\/被用者カナ氏名>/) ||
          pickName(/<被保険者の?カナ氏名>(?:<!\[CDATA\[)?(.*?)(?:\]\]>)?<\/被保険者の?カナ氏名>/);
      }
    ...
    const insurerName =
      pickRootName(/<被保険者の?(?:漢字)?氏名>(?:<!\[CDATA\[)?(.*?)(?:\]\]>)?<\/被保険者の?(?:漢字)?氏名>/) ||
      pickRootName(/<被保険者の?カナ氏名>(?:<!\[CDATA\[)?(.*?)(?:\]\]>)?<\/被保険者の?カナ氏名>/);
