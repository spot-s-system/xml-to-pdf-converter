...
const SHAHO_TITLE_MAP: Array<{
  ...
  // 養育期間標準報酬月額特例申出（N7027001）。フォルダ名は概ね
  // `..._[社保]養育期間標準報酬月額特例・・・` で渡ってくる（手続き名が長く末尾切り詰め）。
  // 識別の安全境界は `[社保]養育期間` まで広げる。
  { pattern: /\[社保\]養育期間/,            title: '厚生年金保険養育期間標準報酬月額特例申出受理通知書' },
...
export async function processOtherFile(
  ...
  //  - 7027001: 養育期間標準報酬月額特例申出受理通知書 — 1 人 1 ファイルで、
  //    XML タグも `<被保険者の漢字氏名>` のように「の」助詞付きの独自レイアウト。
  //    NON_SPLITTABLE 扱いにし、ここで救済する。
