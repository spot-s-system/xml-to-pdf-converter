...
| 労保保険関係成立届（公文書） | `..._[労保]保険関係成立届_..._公文書_*` | `労働保険関係成立届.pdf` |
| 労保名称所在地変更（公文書） | `..._[労保]名称所在地変更_..._公文書_*` | `労働保険名称所在地変更届.pdf` |
| 労保概算保険料申告（継続・公文書） | `..._[労保]概算保険料申告(継続)_..._公文書_*` | `労働保険概算保険料申告書.pdf` |
| 社保新規適用（公文書） | `..._[社保]新規適用_..._公文書_*` | `（社会保険）適用通知書.pdf` |
| 雇保 資格取得 / 資格喪失（離職票交付あり含む）<br/>育児休業出生後休業給付 / 育児時短就業給付 / 育児休業出生時休業給付 | `{seq}_{会社名}_{被保険者名}_[雇保]{該当手続き}_...` | `{被保険者名}様_{元のサフィックス}` |

- 元号変換: 西暦 − 2018（例: 2025 → 令和7年度）
- 労保年度更新のコメントフォルダ（`_コメント_*`）はリネーム対象外
- 雇保資格取得/喪失のリネームは元ファイルが `^\d+_xxx` 形式の場合のみ適用（数字部分を被保険者名に置換）

### PDF生成時の命名フォールバック（[社保]資格取得 / [社保]資格喪失）

XMLから被保険者名や通知書名がうまく取れないケース（DataRoot形式の喪失で `<TITLE>` が無い等）でも、フォルダ名から補完して以下の名前で生成します：

| フォルダ | 出力ファイル名 |
|---|---|
| `..._{被保険者名}_[社保]資格取得_...` | `{被保険者名}様_健康保険・厚生年金保険資格取得確認および標準報酬決定通知書.pdf` |
| `..._{被保険者名}_[社保]資格喪失_...` | `{被保険者名}様_健康保険・厚生年金保険資格喪失確認通知書.pdf` |
| `..._{被保険者名}_[社保]育児休業等申出書_...` | `{被保険者名}様_健康保険・厚生年金保険育児休業等取得者確認通知書.pdf` |
| `..._{被保険者名}_[社保]産前産後休業等申出書_...` | `{被保険者名}様_健康保険・厚生年金保険産前産後休業取得者確認通知書.pdf` |

XML側で名前/タイトルが正しく取れている場合はXMLの値を優先します（フォールバックは欠損時のみ）。
